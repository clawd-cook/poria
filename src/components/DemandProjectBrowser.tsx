...
import type { DemandProjectFile } from "@/lib/types";
...
export function DemandProjectBrowser({
  demandCode,
  demandId,
}: {
  demandCode: string;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const preferred =
          project.files.find((file) => file.name === "PRD_REVIEW.md") ??
          project.files.find((file) => file.name === "PRD.md") ??
          project.files.find((file) => file.name === "TRD.md") ??
          project.files[0];
        setActiveFile(preferred?.name ?? null);
      } catch (err) {
        if (!cancelled) {
  ...
  useEffect(() => {
    if (!demandCode || !activeFile || activeFile === "PRD_REVIEW.md") {
      return;
    }
  ...
  }, [activeFile, demandCode, demandId]);
  ...
  if (!exists) {
    return <EmptyState description="尚未导出文档，请先在流水线执行初始化" />;
  }
  if (files.length === 0) {
    return <EmptyState description="项目目录为空" />;
  }
  ...
  return (
    <div className="flex min-h-[360px] gap-4">
      ...
      <div className="min-w-0 flex-1 overflow-auto">
        {projectDir ? <p className="text-muted-foreground mb-2 text-xs">{projectDir}</p> : null}
