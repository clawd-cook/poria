...
export function PipelineDetail() {
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-slate-400">
          ...
          {detail.has_regressed && (
            <span className="text-amber-400">已回退</span>
          )}
          <a
      ...
      {humanRequest &&
        humanRequest.pipelineId === detail.id && (
          <div className="my-4">
            <HumanLoopCard
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500">
                    {stage.retry_count > 0 && (
                      <span>重试 {stage.retry_count} 次</span>
                    )}
                    {stage.started_at && (
                      <span>
                ...
                {stage.issue && (
                  <p className="mt-1 text-xs text-red-400">{stage.issue}</p>
                )}
              </div>
            ))}
          </div>
