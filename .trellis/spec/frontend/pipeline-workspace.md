...
| Dev with unconfirmed frontend `TRD.md` | Dev `blocked` (`trd_unconfirmed`); HITL 确认 TRD or 跳过确认 (does **not** skip the Dev stage) |
...
- `cargo test -p poria-core -- collect_deploy_mr_urls`
- `cargo test -p poria-core -- human_loop_state`
- `cargo test -p poria-channels -- jme`
- `cargo test -p poria-skills -- human_loop`
